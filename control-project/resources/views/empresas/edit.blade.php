...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-sm uppercase tracking-[0.3em] font-semibold" style="color: #d4af37;">Editar empresa</p>
                <h2 class="text-2xl font-semibold text-white">Atualizar dados da empresa</h2>
            </div>
        ...
        <div class="rounded-3xl border p-6 shadow-sm" style="background-color: #2a2a2a; border-color: #444444;">
            <form action="{{ route('empresas.update', $empresaSolicitante) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
